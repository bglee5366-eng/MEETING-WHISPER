...
export default function AutoStartControl({ open }: { open: boolean }) {
  ...
  const content = <div className="settings-tree" role="tree" aria-label="설정"><strong className="settings-tree-title">설정</strong><details><summary>화면 모드</summary><div className="settings-tree-options">{(["system", "light", "dark"] as Theme[]).map((value) => <button key={value} className={theme === value ? "selected" : ""} onClick={() => chooseTheme(value)}>{radio(theme === value)} {value === "system" ? "시스템 설정" : value === "light" ? "라이트 모드" : "다크 모드"}</button>)}</div></details><details><summary>AI Provider</summary><div className="settings-tree-options">{providers.map((item) => <button key={item.value} className={provider === item.value ? "selected" : ""} onClick={() => chooseProvider(item.value)}><Image src={item.icon} alt="" aria-hidden="true" width={18} height={18} />{radio(provider === item.value)} {item.label}</button>)}</div></details><details><summary>새 회의 시작 방식</summary><p className="settings-tree-help">새 회의를 눌렀을 때 녹음 시작 방식을 선택합니다.</p><div className="settings-tree-options"><button className={autoStart ? "selected" : ""} onClick={() => chooseAutoStart(true)}>{radio(autoStart)} 바로 녹음 시작</button><button className={!autoStart ? "selected" : ""} onClick={() => chooseAutoStart(false)}>{radio(!autoStart)} 시작 화면에서 녹음</button></div></details><details><summary>폰트 크기</summary><div className="settings-tree-options">{fontSizes.map((item) => <button key={item.value} className={fontSize === item.value ? "selected" : ""} onClick={() => chooseFontSize(item.value)}>{radio(fontSize === item.value)} {item.label}</button>)}</div></details></div>;
