...
- 모바일 반응형 화면
- 지원 브라우저에서 Web Speech API 기반 실시간 전사
...
Vercel Serverless Function의 요청 크기 제한을 고려해 음성은 전사에 적합한 낮은 비트레이트로 녹음하며, 약 4MB를 넘는 Blob은 서버로 보내기 전에 차단합니다. 용량 초과 시 녹음을 종료하고 새로 시작하면 됩니다.

장시간 녹음 중에도 전사 가능한 컨테이너 헤더가 유지되도록 MediaRecorder 세그먼트를 주기적으로 교체합니다. 따라서 녹음이 10분을 넘어가도 이후의 반복 구조 요청이 깨진 중간 WebM chunk를 보내지 않습니다.
